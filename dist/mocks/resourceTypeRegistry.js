...
class ResourceTypeRegistry {
     ...
     */
    define(displayName, description, embedding = [], format = 'json', schema = null) {
     ...
     */
    findByDisplayName(partialName) {
        return this.getAll().filter(resourceType => resourceType.displayName.toLowerCase().includes(partialName.toLowerCase()));
    }
    /**
     * Bulk define resource types with a fluent API
     */
    defineMany(definitions) {
        return definitions.map(def => this.define(def.displayName, def.description || '', def.embedding || [], def.format || 'json', def.schema || null));
    }
}
...
const RT = (displayName) => resourceTypeRegistry.get(displayName);
